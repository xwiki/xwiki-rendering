...
public class XHTMLXWikiGeneratorListener extends DefaultXWikiGeneratorListener
{
    ...
    {
        super.beginFigure(params);

        if (isImageFigure(params)) {
            // Push a queue listener so the content of the figure can be inspected.
    ...
    public void endFigure(WikiParameters params)
    {
        if (isImageFigure(params)) {
            ...
            // Try removing a wrapping paragraph from the start and the end that wraps an image. This is because we
            // don't generate this paragraph from XWiki syntax, but it is automatically generated by WikiModel and
            // CKEditor doesn't recognize the figure when it is there, see also
            // https://jira.xwiki.org/browse/XRENDERING-710.
            if (queueListener.size() >= 3) {
                removeWrappingParagraph(queueListener, 0);
            }

            if (queueListener.size() >= 3) {
                removeWrappingParagraph(queueListener, queueListener.size() - 3);
            }

            queueListener.consumeEvents(getListener());
        }

        super.endFigure(params);
    }

    private static void removeWrappingParagraph(QueueListener queueListener, int startIndex)
    {
        if (queueListener.get(startIndex).eventType == EventType.BEGIN_PARAGRAPH
            && queueListener.get(startIndex + 1).eventType == EventType.ON_IMAGE
            && queueListener.get(startIndex + 2).eventType == EventType.END_PARAGRAPH)
        {
            queueListener.remove(startIndex + 2);
            queueListener.remove(startIndex);
        }
    }

    /**
     * @param params the parameters of the figure
     * @return if the class parameter contains the class "image"
     */
    private static boolean isImageFigure(WikiParameters params)
    {
        WikiParameter classParameter = params.getParameter(CLASS_ATTRIBUTE);
        return classParameter != null && Arrays.stream(StringUtils.split(classParameter.getValue()))
            .anyMatch(Predicate.isEqual("image"));
    }
}
